...
上周5晚上，我喝着15年的Macallan威士忌，决定写一个能发现关键(Heartbleed)Bug的静态检测器。它应该作为Clang分析器的一个插件，利用一些有关键Bug的函数来测试它，最后在脆弱的OpenSSL上测试。
...
也很简单，我们的逻辑是去调用isArgUnConstrained方法来判断是否一个没有被约束的值被隐藏，如果在当前路径有一个被污染的符号化的值有不充足的约束，我们就报出Bug。
